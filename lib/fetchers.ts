...
export async function fetchSemanticScholarPapers(): Promise<Paper[]> {
  ...
  // Single broad query for OSC papers
  const query = encodeURIComponent("organic solar cell non-fullerene acceptor");
  const url = `https://api.semanticscholar.org/graph/v1/paper/search?query=${query}&fields=paperId,title,authors,abstract,publicationDate,externalIds,openAccessPdf,url&limit=30&sort=publicationDate:desc`;
